    ...
    @if ($status)
      <div class="w-full text-center space-y-8 animate-in fade-in slide-in-from-right-8 duration-700">
        ...
        <div class="flex justify-center">
          ...
          @else
            <div class="p-6 bg-amber-50 dark:bg-amber-900/20 rounded-full ring-1 ring-amber-100 dark:ring-amber-900/30">
              <flux:icon icon="exclamation-triangle" class="size-24 text-amber-600 dark:text-amber-400"
                variant="solid" />
            </div>
          @endif
        </div>
        ...
        <div class="space-y-6">
          ...
          @else
            <div>
              <h2 class="text-3xl md:text-4xl font-bold text-gray-900 mb-2">{{ $member->name }}</h2>
              {{-- <p class="text-lg font-mono text-gray-500">{{ $member->code }}</p> --}}
                <p class="text-gray-800 mt-5 text-xl font-medium">{{ $member->latestMembership()?->plan_name }}</p>
            </div>

            @if ($status === 'active')
              <div
                class="inline-flex items-center gap-3 px-6 py-2 rounded-full bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 text-lg font-bold">
                {{-- <div class="size-3 rounded-full bg-green-500 animate-pulse"></div> --}}
                MEMBRESÍA ACTIVA
              </div>
              <div class="mt-4">
                {{-- <p class="text-gray-700 mb-4 text-2xl font-medium">{{ $member->latestMembership()->plan_name }}</p> --}}

                {{-- <p class="text-sm font-semibold uppercase text-gray-700">Vence en</p> --}}
                <p class="text-sm font-semibold uppercase tracking-widest text-gray-500">Vence en</p>
            ...
            @elseif($status === 'expired')
              <div
                class="inline-flex items-center gap-3 px-6 py-2 rounded-full bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 text-lg font-bold">
                MEMBRESÍA VENCIDA
              </div>
              <div class="mt-8">
            ...
            @elseif($status === 'no_membership')
              <div
                class="inline-flex items-center gap-3 px-6 py-2 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-lg font-bold">
                SIN MEMBRESÍA
              </div>
