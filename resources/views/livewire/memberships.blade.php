            ...
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-2">
                <flux:button
                  ...
                  variant="outline"
                  icon="chart-bar"
                  wire:click="showHistory({{ $membership->id }})"
                >
                  Ver Historial
                </flux:button>
        ...
        <div class="flex min-h-full items-center justify-center p-4">
          <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:w-full sm:max-w-5xl" wire:click.stop>
            ...
            <div class="flex items-center justify-between px-6 py-3.5 border-b border-gray-200">
              ...
              <flux:button wire:click="closeHistoryModal" variant="ghost">
                <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </flux:button>
                    ...
                    <div class="relative border-l border-gray-200 ml-3 space-y-4">
                      @foreach($selectedMembership->periods as $period)
                        <div wire:key="{{ $period->id }}" class="relative ml-5">
                          <!-- Timeline Dot -->
                          <span class="absolute -left-9 flex h-7 w-7 items-center justify-center rounded-full ring-8 ring-white
                          ...
                          <!-- Content Card -->
                          <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center bg-white border border-gray-200 rounded-lg p-4 py-5 shadow-xs hover:shadow-sm transition-shadow">
                            <div class="flex items-center gap-2">
                            ...
                            <div class="mt-2 sm:mt-0 text-right">
                              <p class="text-lg font-semibold text-gray-900">${{ number_format($period->price_paid) }}</p>
                            </div>
                          </div>
                        </div>
                      @endforeach
                  ...
                  <div class="mt-6 pt-4 border-t border-gray-200">
                    <div class="flex justify-between items-center">
                      ...
                      <span class="text-xl font-semibold text-gray-900">
                        ${{ number_format($selectedMembership->periods->sum('price_paid')) }}
                      </span>
                    </div>
                  </div>
                @else
